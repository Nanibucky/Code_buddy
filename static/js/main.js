...
require(['vs/editor/editor.main'], function() {
    ...
    // Adjust editor settings based on screen size
    ...
    window.addEventListener('resize', function() {
        ...
        if (isMobileNow !== isMobile) {
            // Update editor settings when switching between mobile and desktop
            editor.updateOptions({
                minimap: { enabled: !isMobileNow },
                fontSize: isMobileNow ? 12 : 14,
                lineHeight: isMobileNow ? 20 : 24
            });
        }
    });
});
...
function displayQuestion(data) {
    ...
    if (questionInfo.examples && questionInfo.examples.length > 0) {
        questionInfo.examples.forEach((example, index) => {
            ...
            exampleDiv.classList.add('example', 'mb-4', 'animate-fade-in');
    ...
    if (questionInfo.constraints && questionInfo.constraints.length > 0) {
        questionInfo.constraints.forEach((constraint, index) => {
            ...
            li.classList.add('animate-fade-in', 'mb-2');
    ...
    if (editor) {
        ...
        const savedCode = localStorage.getItem(`code_${data.question_id}`);
        if (savedCode) {
...
document.addEventListener('keydown', function(event) {
    ...
    // Ctrl+Shift+N or Command+Shift+N to generate a new question
    if ((event.ctrlKey && event.shiftKey || event.metaKey && event.shiftKey) && event.key === 'N') {
...
window.addEventListener('load', function() {
    ...
    if (editor) {
        editor.onDidChangeModelContent(() => {
            ...
            const code = editor.getValue();
            if (currentQuestionId) {
                localStorage.setItem(`code_${currentQuestionId}`, code);
            }
